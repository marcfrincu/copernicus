...
//			printf("ok2\n");
			for (int j = 0; j < Objects[i].numMatFaces; j ++)
			{
				...
				glBindTexture ( GL_TEXTURE_2D, Materials[Objects[i].MatFaces[j].MatIndex].texture );				// Bind the texture as the current one
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	...
	printf("loading texture %s" , fullname);
	Materials[matindex].texture = tgaLoadAndBind ( fullname, TGA_LOW_QUALITY );
